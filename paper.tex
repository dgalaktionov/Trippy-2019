...
\begin{itemize}
    ...
    \item \texttt{load\_X$_LT$} The average number of passengers traveling from the stop X to its next stop in the line L within the time range T. Can also be seen as the average load of a transport vehicle.
...
Query &  \acumm & \ctr\\
\hline
\texttt{board\_X$_{LT}$} & O(l) & sum of logs \\
\texttt{alight\_X$_{LT}$} & O(l) & Hard$^*$ \\
\texttt{start\_X$_{LT}$} & - & sum of logs \\
\texttt{end\_X$_{LT}$} & - & sum of logs \\
\texttt{switch\_X$_{LT}$} & - & sum of logs \\
\texttt{from\_X$_{LT}$\_to\_Y$_{LT}$} & - & sum of logs \\
\texttt{board\_L$_T$} & O(1) & sum of logs \\
\texttt{alight\_L$_T$} & O(1) & Approx. T \\
\texttt{start\_L$_T$} & - & sum of logs \\
\texttt{end\_L$_T$} & - & O(X) \\
\texttt{board\_T} & O(l) & SuperHard$^*$ \\
\texttt{alight\_T} & O(l) & MegaHard$^*$ \\
\texttt{start\_T} & - & O(l) \\
\texttt{end\_T} & - & SuperHard$^*$ \\
\texttt{load\_X$_LT$} & O(l) & Hard$^*$ \\
\hline
\end{tabular}
\end{table}
