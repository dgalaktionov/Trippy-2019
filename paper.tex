...
So here goes a picture where we have stops $s \in S$, lines $l \in L$ and journeys $j^l \in J^l$. Will possibly redefine to make clear that they are not sets, and then have subindexes like $s_i$ or $j^l_i$.
...
\begin{itemize}
    \item $lineStop_l(i)$ is the $i$th stop of line $l$
    \item $stopLine_s(i)$ is the $i$th line that makes a stop at the stop $s$
    \item $avgTime_l(i)$ is the average time in seconds that it takes for a vehicle of line $l$ to reach its $i$th stop from the start of a journey
    \item $initialTime_l(i)$ is the starting time of the $i$th journey for line $l$
\end{itemize}

All boring style except for \texttt{initialTimes}, which is compressed with sampled ZSTD.
