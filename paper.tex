...
\subsection{\ttctr}
Version de spire y despues la variante nueva (\ctr).
...
\section{Experiments}
\label{sec:exp}
In this section we discuss the practical performance of our structures. To evaluate them, we have run randomly generated queries against \acumm, \ttctr~and \ctr, with several configurations to study the trade-off between compression and query efficiency. For the \texttt{CSA} structures from \ttctr~and \ctr, we experiment with the $t_\Psi$ (sampling interval) factors of 32, 128 and 512. For the \texttt{WM} present in \ttctr, as well as in the two \texttt{WM} from \ctr, we analyze the compression achieved with the RRR bitvector described in \cite{Raman:2002:SID:545381.545411}, using block sizes of 32, 64 and 128, and compare it against a plain bitvector with a rank structure. \marginpar{\tiny Do we have any similar trade-off for \acumm?}

\subsection{Experimental dataset}
Using real GTFS descriptions of bus routes and schedules, we generated a synthetic dataset of user trips that aims to realistically imitate real user behaviour over a month. In this work, we combined the GTFS obtained for the networks of urban\footnote{\url{http://www.emtmadrid.es}} and interurban\footnote{\url{http://www.crtm.es}} buses for the city of Madrid. My head hurts right now, but the steps to generate user trips are:

\begin{enumerate}
    \item Extract stops an lines. A reversed direction makes a new line.
    \item Connect stops that are on walking distance (100 meters) from each other, or appear sequentially on the same line.
    \item Parse the schedules for bus trips.
    \item Generate a month of journeys from the schedules. This is used both to generate user trips and to build our common structures.
    \item Generate user trips starting from a random stop and with a binomial distribution for switching lines, where the user is allowed to wait up to 30 minutes.
    \item Profit.
\end{enumerate}

\subsection{Space requirments}
Discuss why we are getting these sizes, and compare. Missing \acumm~sizes!
...
\begin{table}
    \caption{Space requirements for the CSA (a) and the WM (b) from \ttctr}
...
\begin{table}
    \caption{Space requirements for the CSA (a), the WMJ (b) and the WML (c) from \ctr}
    \begin{subtable}[t]{.48\linewidth}
    \vspace{-12pt}
    \caption{\footnotesize (a)}
    \vspace{-12pt}
...
\vspace{-12pt}
\end{subfigure}
\caption{Comparison of \texttt{start\_X$_{LT}$} (a) and \texttt{end\_X$_{LT}$} (b) queries, with all variants}
\end{figure}
