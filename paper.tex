...
\texttt{board\_L$_T$} & O(1) & $O(ns \times \ctrq)$\tnote{be} \\
\texttt{alight\_L$_T$} & O(1) & $O(ns \times \ctrq)$\tnote{bce} \\
\texttt{start\_L$_T$} & - & \ctrq\tnote{b} \\
\texttt{end\_L$_T$} & - & $O(ns \times log(|L|\times\bar{|J^l|}))$\tnote{ef} \\
\texttt{board\_T} & $O(l)$\tnote{a} & Hard\tnote{d} \\
\texttt{alight\_T} & $O(l)$\tnote{a} & Hard\tnote{cd} \\
\texttt{start\_T} & - & $O(|L| \times log\bar{|J^l|})$\tnote{f} \\
...
\item[b] The complexity of a \ctrq~query varies with the restrictions.
\item[c] May include false positives.
\item[d] Not practical to solve with the indexing capabilities of \ctr.
\item[e] $ns$ is the number of stops in the line L.
\item[f] $|L|$ is the total number of lines represented, and $\bar{|J^l|}$ is the average number of jcodes per line.
\end{tablenotes}
\end{threeparttable}


In \ctr, the only alighting stops that are explicitly represented are the final stops. To obtain an approximated count of the rest of them, we would need to extract and examine all the occurrences for board\_W$_{LT}$ for every W that could have been boarded before X, as the worst-case time complexity of solving with the index operations is prohibitive.
