...
\newcommand{\acumm}{T-Matrices} % T is for Trips!
...
\begin{document}
%
\title{Improved structures to solve aggregated queries for trips over public transportation networks \thanks{Funded in part by European Union`s Horizon 2020 research and innovation programme
...
%
\begin{abstract}
In order to efficiently store and analyze large datasets of passenger trips over public transportation networks, one promising approach is the use of a representation based of compact data structures.

In this work we propose a solution based on two complementary representations, \acumm~and~\ttctr, with an alternative for the later called \ctr. Overall, with our representations we were able to store a dataset of ten million trips in around 100 MB, supporting a wide range of queries in the order of microseconds.

\ojo{sin acabar}
Storing and efficiently analyzing large datasets of passenger's trips over public transportation networks brings the ability to gather the actual user's demands and consequently sets the basis to balance both the offer and such demands. We show that the use of compact data structures makes up a promising representation of passenger's trips that not only permits us to reduce the space needed to store them, but also provides efficient access operations at query time.

In this work, we propose a solution based on two complementary representations, named \acumm~and~\ttctr. The former excels at operations XXX, whereas the later excels at operations YYY. In addition, we propose a variant of \ttctr\ named \ctr\ that performs better at operations XXX at the cost of losing performance at ... with an alternative for the later called \ctr. Overall, with our representation we were able to represent a dataset of ten million trips in around 100 MB, while supporting a wide range of queries in the order of microseconds. \ojo{non sei se se pode explicar a grandes rasgos e en pouco espazo que fan as 3 tecnicas, e cales son as suas vantaxes}
